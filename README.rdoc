...
and email to other recipients (like the real users in the production database
you copied to a test server) is suppressed and be redirected to `forward_to`
email (if any).

You can also disable the safety_mailer dynamically by setting the environment
variable `DISABLE_SAFETY_MAILER=true`
